...
def animal_ayant_appartenu_par_client(conn, prenom_client, nom_client):
    cur = conn.cursor()
...
def is_valid_date(date_string):
    # On vérifie que la chaîne de caractères correspond à la regex
    if not re.match(r'^\d{4}-\d{2}-\d{2}$', date_string):
        return False

    # On extrait les différentes parties de la date
    ...
    # On vérifie que le jour est compris entre 1 et 31
    if not (1 <= day <= 31):
        return False

    # Si on a passé toutes ces vérifications, c'est que la date est valide
    return True
...
def main():
    ...
    while continuer:
        ...
        if choix == 1:  # Insertions
            ...
            print("4 : Compter le nombre d'animaux traités par groupe")
            print(
            ...
            if choix == 1:
                ...
                prenom_client = str(input("Le prénom de client : \n"))
                nom_client = str(input("Le nom de client : \n"))
                animal_actuel_par_client(conn, prenom_client, nom_client)

            elif choix == 7:
                prenom_client = str(input("Le prénom de client : \n"))
                nom_client = str(input("Le nom de client : \n"))
                animal_ayant_appartenu_par_client(conn, prenom_client, nom_client)
